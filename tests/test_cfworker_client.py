...
from sms_tool.providers import cfworker_client
from sms_tool.providers.cfworker_client import CFWorkerMailboxClient
...
class CFWorkerMailboxClientTests(unittest.TestCase):
    def test_create_mailboxes_returns_addresses_from_worker_response(self):
        ...
        with patch.object(cfworker_client.curl_requests, "post", return_value=CreateMailboxResponse()) as post:
    ...
    def test_create_mailboxes_never_synthesizes_addresses_after_provider_failures(self):
        ...
        with patch.object(cfworker_client.curl_requests, "post", return_value=EmptyCreateMailboxResponse()), \
             patch.object(cfworker_client.curl_requests, "get", return_value=EmptyCreateMailboxResponse()):
    ...
    def test_mixed_plain_and_encoded_rfc2047_subject_is_decoded(self):
        ...
        self.assertEqual(cfworker_client._decode_header_value(subject), "OpenAI - Access Deactivated\n\n[C-q4RKCjo6DIKv]")
    ...
    def test_admin_email_list_uses_proxy_filters_recipient_and_exposes_otp_body(self):
        ...
        with patch.object(cfworker_client.curl_requests, "get", return_value=FakeResponse()) as get:
    ...
    def test_missing_recipient_messages_are_not_assumed_to_match_target_mailbox(self):
        ...
        with patch.object(cfworker_client.curl_requests, "get", return_value=MissingRecipientEndpointResponse()):
    ...
    def test_normalized_message_does_not_synthesize_target_recipient(self):
        msg = cfworker_client._normalize_message(
            {
                "message_id": "global-latest",
                "subject": "Your temporary ChatGPT verification code",
                "extracted_json": '[{"value":"202123"}]',
    ...
    def test_target_endpoint_exposes_otp_from_raw_text_when_extracted_json_is_empty(self):
        ...
        with patch.object(cfworker_client.curl_requests, "get", return_value=RawTextEndpointResponse()):
            messages = client.fetch_messages("target@edu.liziai.cloud", limit=5)
    ...
    def test_fetch_uses_urllib_fallback_when_curl_request_fails(self):
        ...
        with patch.object(cfworker_client.curl_requests, "get", side_effect=RuntimeError("curl timeout")):
