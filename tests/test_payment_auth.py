...
class PaymentAuthTests(unittest.TestCase):
    def test_live_token_passes_without_relogin(self):
        ...
        with patch.object(payment_auth, "load_account_seed", return_value=(account, "session.json")), \
             ...
             patch("sms_tool.accounts.account_liveness.probe_account_liveness", return_value={"ok": True, "status_code": 200}), \
             patch("sms_tool.accounts.account_recovery.relogin_codex_account") as relogin:
    ...
    def test_401_oauth_refresh_reloads_persisted_token(self):
        ...
        with patch.object(payment_auth, "load_account_seed", side_effect=[(before, "session.json"), (after, "session.json")]), \
             patch("sms_tool.accounts.account_recovery.is_permanently_deactivated", return_value=False), \
             patch("sms_tool.accounts.account_liveness.probe_account_liveness", return_value={"ok": False, "status_code": 401}), \
             patch("sms_tool.accounts.account_recovery.relogin_codex_account", return_value={
