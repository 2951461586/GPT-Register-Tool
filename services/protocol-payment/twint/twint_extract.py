...
def normalize_proxy_url(proxy: str) -> str:
    ...
    Body was byte-for-byte identical to ``ideal_qr_extract``'s; both now share
...
def default_proxy_scheme() -> str:
    ...
    return default_scheme_from_env("TWINT_PROXY_DEFAULT_SCHEME")
...
def twint_expected_proxy_countries(group: str) -> set[str]:
    return shared_geo.expected_proxy_countries(
        group,
        env_prefix="TWINT",
        default_checkout_country=lambda: TWINT_BOOTSTRAP_COUNTRY,
        default_country=lambda: TWINT_PROVIDER_COUNTRY,
        default_provider_countries=lambda: TWINT_PROVIDER_COUNTRY,
    )
...
def maybe_check_proxy_geo(checkout_proxy: str, provider_proxy: str) -> None:
    ...
    Same rationale as ideal's: the derived chain shares one exit IP, so the
    check probes the seed's exit country once. Gated behind
    ``TWINT_PROXY_GEO_CHECK`` — turning it on is a behaviour change.
    """
...
def _redact_text(text: str, limit: int | None = None) -> str:
    ...
    (``TWINT_DUMP_LIMIT`` vs the twint one), and a default would silently pick one.
    """
...
def dump_http(
    ...
    Every knob is this module's own: the switch is ``TWINT_DUMP`` (ideal/blik
    read ``IDEAL_DUMP``), dumps land in this module's ``DUMP_DIR``, the index
    numbers this extractor's files only, and redaction uses this module's
    registry.
    """
    shared_dump_http(
        response,
...
def load_token() -> tuple[str, str]:
    ...
    ``env_names`` is passed explicitly and must stay as written: twint uses
    ``TWINT_TOKEN`` where ideal/blik use ``IDEAL_TOKEN``, and the first hit in
    the tuple wins.  Do not "unify" these with the other extractors.
    """
    return shared_load_token(
        env_names=("PP_TOKEN", "TWINT_TOKEN"),
...
def stripe_confirm_return_url(cs_id: str, checkout: dict[str, str], stripe_hosted_url: str) -> str:
    return shared_stripe_flow.stripe_confirm_return_url(
        cs_id, checkout, stripe_hosted_url, normalize_country=normalize_country, default_country="CH"
    )
...
def is_preferred_proxy(group: str, proxy: str) -> bool:
    ...
    (``TWINT_PROXY_SCORE`` vs the other one); a default would silently make one answer to
    the other's switch.
    """
    return shared_is_preferred_proxy(
        group,
        proxy,
        score_env="TWINT_PROXY_SCORE",
        env_bool=env_bool,
