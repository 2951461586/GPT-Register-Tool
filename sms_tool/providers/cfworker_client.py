"""Low-level Cloudflare-Worker mailbox HTTP client.

Naming contract for ``sms_tool/providers/``: this package holds two layers --
low-level protocol clients end in ``_client`` (this module, ``smailr_client``,
``outlook_imap_client``), while the registration-facing mailbox flows that
compose them are named ``mailbox_*`` (``mailbox_cfworker``,
``mailbox_smailr``).  Do not import ``mailbox_*`` from here; that inverts the
layering.
"""
