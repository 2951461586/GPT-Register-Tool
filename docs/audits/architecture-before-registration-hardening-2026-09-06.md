# Architecture snapshot before registration hardening (2026-09-06)

Historical snapshot preserved during the documentation split. File sizes, line
anchors and outstanding findings below are not current implementation claims.
See [current architecture](../architecture.md) and [directory map](../directory-map.md).
...
| `registration_drivers/` | 18 | ~4100 | **45** | **14** | 36+ | 11+ | 0.23 | 全仓扇入最高；2026-09-05 起分层拆入 `browser_flow/`（下文为 09-06 实测，本行 09-02 快照数字仅存档） |
...
3. `dom_fields.py` 的静默失败沿用了单体时期的行为：选择器失配统一返回 `False`，
   ...
   `docs/audits/audit-2026-09-02-round5-summary.md`）。拆分没有改变这一点。
...
**职责边界。** 拥有浏览器注册的驱动抽象与各驱动实现：驱动名归一化（`base.py`
注册表是单一事实源）、浏览器上下文、反指纹注入、外部反检测浏览器的 CDP 接入
（Roxy / Cloak / Camoufox / AdsPower）、以及分层拆分后的 Playwright 注册流程包
`browser_flow/`。

不负责：注册流程的阶段编排（归 `registration_handlers.py` / `registration_state.py`）、
并发闸门（归 `registration_concurrency.py`）、注册结果持久化（归 `store/`）。

**分层结构（2026-09-05 拆分，2026-09-06 实测行数）。**
`playwright.py` 退化为 27 行公共 API 薄壳（只 re-export 三个入口符号，刻意不导出
私有函数，让旧 patch 目标响亮失败）；真实实现全部在 `browser_flow/`：

| 文件 | 行数 | 角色 |
| --- | --- | --- |
| `browser_flow/orchestrator.py` | 641 | 入口编排 `run_browser_registration` 与结果装配 |
| `browser_flow/page_state.py` | 422 | 页面状态判定与等待（挑战 / OTP / 资料 / 取消哨兵） |
| `browser_flow/session.py` | 401 | session payload、AT 探测、诊断、TOTP 绑定 |
| `browser_flow/form_steps.py` | 388 | 邮箱 / 密码 / OTP / 资料表单语义步骤 |
| `browser_flow/dom_fields.py` | 332 | DOM 原语（locator / 点击 / 文本），完全独立 |
| `browser_flow/flow_steps.py` | 214 | 会话作用域（进程池）、OTP 轮询、流程重启 |
| `browser_flow/context.py` | 53 | 每次注册的上下文准备 |
| `browser_flow/recovery.py` | 39 | 纯策略（可测试的重试/超时判定） |
| `external_sessions.py` | 855 | 反检测浏览器会话（Roxy / Cloak / Camoufox / AdsPower） |
| `browser_session.py` | 373 | 本地 Playwright 浏览器会话 |
| `base.py` | 140 | 驱动注册表 + `BrowserRegistrationError` |
| `stealth.py` | 110 | 反指纹注入 |
| `roxy.py` / `camoufox.py` / `cloak.py` | 19 / 12 / 12 | 三行包装（分派在 `registration.py`） |
| `__init__.py` | 5 | 出口 |

**对外接口。** `registration_drivers/__init__.py` 的 `__all__` 有 **4 个符号**：
`BROWSER_REGISTRATION_DRIVERS`、`RegistrationDriver`、`driver_capabilities`、
`normalize_registration_driver`——全部来自 `base.py`，实现模块一个都不导出。
`playwright.py` re-export `run_browser_registration` / `run_playwright_registration` /
`build_browser_session_file`；`browser_flow/__init__.py` 同样只导出这三个入口。

**Patch 失效面（2026-09-06 已根除）。** 拆分初期各层用 from-import 持有彼此的
私有函数副本，patch 必须打遍所有副本才生效，曾需要 170 行测试专用
`tests/browser_flow_patch.py` + 203 行守卫测试兜底。现在 `browser_flow` 内部
调用点一律走**定义模块的命名空间**（`form_steps._fill_email(...)`），包级
`__init__` 不再 re-export 私有符号；patch 只需、也只能打在源模块上，打错位置
直接 AttributeError。上述两个补丁基建文件已删除。

**依赖关系。** `browser_flow` 分层单向：`dom_fields`（独立）← `page_state` ←
`form_steps` ← `flow_steps` ← `orchestrator`；`session` 挂在 dom_fields 之下被
orchestrator 消费。`flow_steps` 函数内 import `browser_pool`（父包模块），
`browser_pool` 顶层 import 本子包——方向已知，靠延迟导入避免环。

**已知的债。**

1. `run_browser_registration`（orchestrator.py，约 565 行）仍是单一编排函数，
   混合了导航、状态机、OTP 重试、资料补全、session 抓取、2FA、identity 持久化
   等职责；拆分时以"先分层、后拆函数"为顺序，函数级拆分尚未做。
2. `roxy.py` / `camoufox.py` / `cloak.py` 三个 wrapper 无生产调用（分派在
   `registration.py` 的字符串归一化）；保留作为文档化的驱动入口示例。
3. `BrowserRegistrationError`（`base.py`）是事实上的公共契约（`external_sessions.py`
   至少 6 处抛它），但不在 `__init__.py` 的 `__all__` 里，调用方只能
   `from sms_tool.registration_drivers.base import BrowserRegistrationError`。
4. `camoufox` 未实现 `release_account_context` / `renew_account_context`
   （显式 `None`），进程池对它退化为全量重启。
...
2. **env 契约全仓无一处声明。** `adapters.py:132-158` 是一条按 `spec.key` 的 4 分支 if 链，
   ...
   两边同时对齐。**建议的收敛方向见 `docs/audits/audit-2026-09-02-round5-decoupling.md` §4.4 的
