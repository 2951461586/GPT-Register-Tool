...
| Account liveness and recovery | `accounts/account_liveness.py`, `accounts/account_recovery.py`, `accounts/account_scan.py`, `accounts/account_promotion.py`, `promotion_states.py` | Canonical side-effect-free quota probe, explicit OAuth recovery/persistence, batch account scan, and the plan/promotion (优惠) probe; `promotion_states.py` is the dependency-free machine-state vocabulary (`promotion_state`) shared with `store`, `desktop_read` and the cross-language contract fixture; does not switch Workspace state. |
