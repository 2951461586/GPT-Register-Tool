...
| Quantity | 2026-09-22 | 2026-09-23 |
| --- | --- | --- |
| Tracked `sms_tool/**.py` | 240 | 240 |
| Named by at least one row | 180 (75%) | **211 (88%)** |
| Named by no row | **60** | **29** |
| Named by more than one row | 5 | 5 (unchanged: `storage.py` ×3; `commands/email_change.py`, `payment_link_manager.py`, `promotion_states.py`, `sub2api_import.py` ×2) |
...
The 29 unnamed files are real modules with real owners, grouped as:
`sentinel/` ×4 · `paypal_link/` ×3 · `registration_checkpoint.py` +
`registration_finalize.py` · and 20 single files (`upi_link.py`, `desktop_ipc.py`,
...
`token_telemetry.py`, `sms_utils.py`, `pp_link_helpers.py`, and the three
