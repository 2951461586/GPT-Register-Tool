...
| `SmsWorkbench/` | Desktop UI | WPF launcher, account grid, themed dialogs, selected-email seam, account liveness, mailbox-change dialog, batch protocol-payment dialog, fixed non-payment proxy launcher, read-only SMS provider catalog adapter, local command planning/result presentation, desktop publish scripts | UI starts CLI commands; payment and mailbox-change protocol logic stays in `sms_tool`. |
...
| Mailbox and phone inventory | `mailbox.py`, `mailbox_service.py`, `mailbox_strategies.py`, `mailbox_types.py`, `mailbox_parsers.py`, `providers/mailbox_*.py`, `providers/*_client.py` (`cfworker_client`, `smailr_client`, `outlook_imap_client`), `mail_otp.py`, `smsbower.py`, `nexsms.py`, `phone_reuse.py`, `phone_proxy.py`, `sms_provider_adapter.py`, `sms_providers.py` | Acquire/poll mailboxes or phone activations. The immutable registry resolves fetch, poll and credential capabilities; `mailbox_errors.py` owns terminal polling classification. All active mailbox provider implementations live under `sms_tool/providers/`; the top-level `mailbox_<provider>` files are aliasing facades only. `mailbox.py` itself is **not** one of those facades: it is the implementation and the private-symbol hub, which is why its cross-module consumers are ratcheted by Boundary Rule 16 rather than assumed away. **Naming contract inside `providers/`:** low-level wire clients end in `_client`, registration-facing flows that compose them are named `mailbox_*`; a `*_client` module must never import a `mailbox_*` module. **SMS-provider naming contract:** `sms_providers.py` is the **single source of truth** for the provider vocabulary (keys, aliases, default endpoints, API-key env names, `client_available`) and `sms_provider_adapter.py` is the 43-line adapter *contract* that consumes it. The `_adapter` suffix is the load-bearing part: add a provider to the registry, add lifecycle surface to the adapter — never the other way round. **Two protocol families, one vocabulary:** `smsbower.py` speaks the sms-activate handler family (`protocol=sms_activate_handler`, lifecycle keyed on `activationId`), while `nexsms.py` speaks the NexSMS JSON REST family (`protocol=nexsms_json`, lifecycle keyed on the **phone number** — that vendor issues no `activationId` and has no `setStatus`). `sms_providers.IMPLEMENTED_PROTOCOLS` is the set this repository ships a client for, so the hand-written `client_available` flag must equal the derived `has_client`. `tests/test_sms_provider_registry.py` pins the registry (including that `client_available == has_client` equality), `tests/test_sms_provider_adapter_pure.py` pins the adapter, `tests/test_settings_catalog_provider_parity.py` pins the C# mirror, and `tests/test_nexsms_client_pure.py` pins the JSON family's wire contract. Chongzhi was removed; unsupported legacy lines are skipped. | `git ls-files sms_tool/mailbox.py sms_tool/mailbox_service.py sms_tool/mailbox_strategies.py sms_tool/mailbox_types.py sms_tool/mailbox_parsers.py "sms_tool/providers/mailbox_*.py" sms_tool/providers/cfworker_client.py sms_tool/providers/smailr_client.py sms_tool/providers/outlook_imap_client.py sms_tool/mail_otp.py sms_tool/smsbower.py sms_tool/nexsms.py sms_tool/phone_reuse.py sms_tool/phone_proxy.py sms_tool/sms_provider_adapter.py sms_tool/sms_providers.py` |
| Mailbox facades and polling primitives | `mailbox_cfworker.py`, `mailbox_gmail.py`, `mailbox_graph.py`, `mailbox_icloud_url.py`, `mailbox_remail.py`, `mailbox_smailr.py`, `outlook_imap.py` (facades) · `mailbox_errors.py`, `mailbox_poll.py`, `mailbox_quarantine.py` (real modules) | The seven top-level provider files are **compatibility facades only** — every implementation lives in `providers/mailbox_*`; add behaviour there, not here. The other three are not facades: `mailbox_errors.py` owns the provider-independent **terminal-error vocabulary** understood by every OTP poller, `mailbox_poll.py` owns the shared **settle-stability polling loop** (deadline / interval / settle) that `_poll_email_otp` and the provider pollers both use, and `mailbox_quarantine.py` owns the **token-free quarantine** for credentials that are no longer usable. | `git ls-files sms_tool/mailbox_cfworker.py sms_tool/mailbox_errors.py sms_tool/mailbox_gmail.py sms_tool/mailbox_graph.py sms_tool/mailbox_icloud_url.py sms_tool/mailbox_poll.py sms_tool/mailbox_quarantine.py sms_tool/mailbox_remail.py sms_tool/mailbox_smailr.py sms_tool/outlook_imap.py` |
...
| Account domain core | `accounts/__init__.py`, `accounts/account_models.py`, `accounts/account_identity.py`, `accounts/account_events.py`, `accounts/account_health.py`, `accounts/account_health_queue.py`, `accounts/account_lifecycle.py` | `account_models.py` is the typed account/session boundary used by persistence and workflow results; `account_identity.py` allocates per-account network and protocol identity once at registration and rebuilds it per worker; `account_events.py` publishes post-persistence domain facts **without importing a mailbox provider** (provider side effects stay behind the dispatcher); `account_health.py` is the plan/liveness **result contract** and `account_health_queue.py` the durable deduplicated queue that owns claim / lease / heartbeat / scheduling only — both are documented in [`current/account-health.md`](current/account-health.md), which is also the promotion (优惠) contract; `account_lifecycle.py` holds typed lifecycle operations shared by the CLI and desktop adapters. `accounts/__init__.py` records why these sit in a package instead of flat `account_*.py` at the root. | `git ls-files sms_tool/accounts/__init__.py sms_tool/accounts/account_models.py sms_tool/accounts/account_identity.py sms_tool/accounts/account_events.py sms_tool/accounts/account_health.py sms_tool/accounts/account_health_queue.py sms_tool/accounts/account_lifecycle.py` |
| Payment contracts, catalog and stage gates | `payment_catalog.py`, `payment_contracts.py`, `payment_adapters.py`, `payment_egress.py`, `payment_operation.py`, `payment_batch_setup.py`, `paypal_extract.py`, `paypal_fingerprints.py`, `paypal_authorization.py`, `paypal_authorization_queue.py` | `payment_catalog.py` is the versioned method catalog shared with the desktop; `payment_contracts.py` owns typed result contracts and retry gates; `payment_adapters.py` is the typed seam to the subprocess extractors; `payment_egress.py` owns the **strong egress-country gate** — stage proxies are probed through themselves and the observed exit must match the route plan *before* an extractor or capability probe starts; `payment_operation.py` owns durable idempotency and recovery; `payment_batch_setup.py` holds pure setup helpers. PayPal side: `paypal_extract.py` is the three-stage proxy link extractor (moved verbatim out of `gen_pp_link.py`), `paypal_fingerprints.py` centralises the UA/version strings so the PayPal modules cannot drift apart, `paypal_authorization.py` is a read-only authorization-response parser, and `paypal_authorization_queue.py` persists the approval-link → BA hand-off so a restart cannot lose a generated link. | `git ls-files sms_tool/payment_catalog.py sms_tool/payment_contracts.py sms_tool/payment_adapters.py sms_tool/payment_egress.py sms_tool/payment_operation.py sms_tool/payment_batch_setup.py sms_tool/paypal_extract.py sms_tool/paypal_fingerprints.py sms_tool/paypal_authorization.py sms_tool/paypal_authorization_queue.py` |
| Proxy authority, lanes, health and bridge | `proxy_entry.py`, `proxy_routing.py`, `proxy_health.py`, `proxy_bridge.py` | `proxy_entry.py` is the **single authority** for parsing, rebuilding, retargeting and rotating proxy strings (Boundary Rule 13 — `phone_proxy` and `paypal_proxy` are thin wrappers around it, and a fourth parser is a defect); `proxy_routing.py` owns the explicit lanes (registration / protocol work / account health) and the ordered operation-candidate list that callers must **not** rebuild; `proxy_health.py` is the process-shared health tracker whose writes go through an OS file lock; `proxy_bridge.py` starts the in-process `socks5h://127.0.0.1:<port>` bridge for a single upstream when a browser needs a local endpoint. | `git ls-files sms_tool/proxy_entry.py sms_tool/proxy_routing.py sms_tool/proxy_health.py sms_tool/proxy_bridge.py` |
...
**Measured 2026-09-23** (after four family rows were added — see the note below):

| Quantity | 2026-09-22 | 2026-09-23 (scan tree) | 2026-09-23 (current) |
| --- | --- | --- | --- |
| Tracked `sms_tool/**.py` | 240 | 240 | **245** |
| Named by at least one row | 180 (75%) | **211 (88%)** | **213 (87%)** |
| Named by no row | **60** | **29** | **32** |
| Named by more than one row | 5 | 5 | 5 (unchanged: `storage.py` ×3; `commands/email_change.py`, `payment_link_manager.py`, `promotion_states.py`, `sub2api_import.py` ×2) |

⚠️ **Why there are two 2026-09-23 columns.** The middle column is a **working-tree** measurement: at scan time the four modules the same commit then introduced (`sms_providers.py`, `mailbox_pool_writer.py`, `page_truth.py`, `browser_profile_reclaim.py`) were still **untracked**, so `git ls-files` could not see them and the denominator stayed 240. Committing them moved the denominator to 244 (only `sms_providers.py` is named by a row, hence 212 / 32), and adding `nexsms.py` moves it to 245 (213 / 32).

The four rows added on 2026-09-23 — *Mailbox facades and polling primitives*,
*Account domain core*, *Payment contracts, catalog and stage gates*, and
*Proxy authority, lanes, health and bridge* — attribute 31 files and add **no new
cross-row duplicate**. Note they are written as **explicit file lists**, not new
globs: a glob's expansion is not stable across tools (git pathspec `*` crosses
`/`; shell and `pathlib` `*` do not), so the same pattern measured 113 / 127 / 180
depending on who expanded it. An explicit list cannot drift that way.

⚠️ **The table is still not exhaustive and this section does not pretend it is.**
The 32 unnamed files are real modules with real owners, grouped as:
`sentinel/` ×4 · `paypal_link/` ×3 · `registration_checkpoint.py` +
`registration_finalize.py` · and 23 single files (`upi_link.py`, `desktop_ipc.py`,
`config_usage.py`, `codex_oauth.py`, `chatgpt_bootstrap.py`, `backoff.py`,
`batch_circuit_breaker.py`, `sanitizer.py`, `http_utils.py`, `humanize.py`,
`logging_setup.py`, `env_loader.py`, `failure_registry.py`, `diagnostics.py`,
`token_telemetry.py`, `sms_utils.py`, `pp_link_helpers.py`, `page_truth.py`,
`mailbox_pool_writer.py`, `browser_profile_reclaim.py`, and the three
package `__init__.py` files). They are simply not yet attributed to a row.
Do not quote a coverage percentage as a gate until that list is empty —
attribute them first, then the number becomes meaningful.
...
| `BackendResultInterpreter.cs` | Interpret backend execution results and proxy-test JSON into typed outcomes (success/timeout/cancelled). Also owns the **account-scan result surface** (`IsAccountScanResultTask` / `AccountScanResultTitle` / `ScanResultError`), absorbed from the former `AccountScanResultInterpreter` when that file was deleted on 2026-08-31 as zero-reference dead code — the presentation half lives in `MainWindow.Export.cs::ShowAccountScanResultDialog`. |
...
2. If it is a desktop button/dialog, put UI code in `SmsWorkbench/` and call the
   ...
   module such as `SmsProviderCatalogClient.cs`, not in a `MainWindow` handler.
