...
| Account liveness and recovery | `accounts/account_liveness.py`, `accounts/account_recovery.py`, `accounts/account_scan.py`, `accounts/account_promotion.py`, `accounts/account_terminal.py`, `promotion_states.py` | Canonical side-effect-free quota probe, explicit OAuth recovery/persistence, batch account scan, and the plan/promotion (优惠) probe; `accounts/account_terminal.py` (terminal-status vocabulary) and `promotion_states.py` (promotion machine state) are the dependency-free cross-layer vocabularies; does not switch Workspace state. |
