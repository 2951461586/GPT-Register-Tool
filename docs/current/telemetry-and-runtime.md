...
- `sms_tool.log` — the operator log, one normalized line per record:
  `HH:MM:SS [*] [模块] 消息`. Level markers are `[*] [!] [x] [.]`, logger names
  map to module labels (注册 / 浏览器注册 / 账号测活 / 优惠检测 / 一键接码 /
...
duration is settled when leaving the current stage, and a successful run emits
one `completed` terminal event. Account identifiers in progress and IPC records
use stable hashes; operator-facing email labels are masked.
